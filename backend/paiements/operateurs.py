# ============================================================
# backend/paiements/operateurs.py
#
# RÔLE : Clients pour MTN MoMo et Orange Money Cameroun.
#
# COMPRENDRE LE FLOW :
#   1. Votre app demande à MTN de collecter l'argent
#   2. MTN envoie un SMS à l'utilisateur
#   3. L'utilisateur valide sur son téléphone
#   4. MTN appelle votre webhook pour confirmer
#   5. Votre app confirme la réservation + envoie notification
# ============================================================

import uuid, requests, base64, logging
...
class MTNMoMoClient:
    ...
    SANDBOX (tests gratuits) :
      ...
      Numéros test : 46733123450 (succès), 46733123451 (échec)

    PRODUCTION :
      URL : https://proxy.momoapi.mtn.com
      Commission : 0.75% du montant
    """
    ...
    def get_access_token(self):
        """
        ÉTAPE 1 du flow MTN : obtenir un token d'accès.

        Le token est valide 1 heure.
        En production, vous pourriez le mettre en cache (Redis).
        """
        if not self.api_user or not self.api_key:
            logger.error("MTN MoMo : API_USER ou API_KEY manquant dans .env")
            return None

        credentials = base64.b64encode(
            f"{self.api_user}:{self.api_key}".encode('utf-8')
        ).decode('utf-8')
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.debug("MTN MoMo token obtenu")
                return token
            else:
                logger.error(f"MTN token erreur {r.status_code}: {r.text[:200]}")
                return None
    ...
    def request_to_pay(self, montant, telephone, reference_interne):
        """
        ÉTAPE 2 du flow MTN : demander le paiement.

        MTN envoie ensuite un USSD push (*126#) sur le téléphone
        du client pour qu'il valide avec son PIN.

        ENTRÉE :
          montant          : int (ex: 3500)
          telephone        : str sans code pays (ex: "699000000")
          reference_interne: UUID unique de la transaction
        ...
        if not token:
            return {
                'succes' : False,
                'message': 'Impossible d\'obtenir le token MTN. Vérifiez MTN_MOMO_API_USER et MTN_MOMO_API_KEY dans .env'
            }
        ...
        try:
            r = requests.post(
                ...
                json = {
                    "amount"      : str(int(montant)),  # MTN attend une string
                    "currency"    : "XAF",               # Franc CFA
                    "externalId"  : ref,                 # Votre référence unique
                    "payer"       : {
                        "partyIdType": "MSISDN",
                        "partyId"    : f"237{telephone.replace(' ', '')}",
                    },
                    "payerMessage": "Paiement BusCam - Billet de bus",
                    "payeeNote"   : f"Billet {ref[:8].upper()}",
                },
                headers = {
                    **self._headers_base(),
                    'Authorization'  : f'Bearer {token}',
                    'X-Reference-Id' : ref,          # Identifiant unique de la transaction
                    'X-Callback-Url' : self.callback_url,  # URL webhook
                },
                timeout = 20,
            )

            # 202 = Accepted (demande reçue, en attente de validation)
            if r.status_code == 202:
                logger.info(f"MTN request_to_pay accepté: {ref}")
                return {
                    'succes'   : True,
                    'reference': ref,
                    'message'  : f"Demande envoyée. L'utilisateur doit valider avec son PIN MTN MoMo.",
                }
            else:
                logger.error(f"MTN request_to_pay erreur {r.status_code}: {r.text[:200]}")
                return {'succes': False, 'message': f"Erreur MTN ({r.status_code})."}

        except requests.Timeout:
            return {'succes': False, 'message': 'MTN MoMo : timeout. Réessayez.'}
        except requests.RequestException as e:
            logger.error(f"MTN request_to_pay exception: {e}")
            return {'succes': False, 'message': f'Erreur réseau MTN: {str(e)}'}

    def get_payment_status(self, reference_interne):
        """
        Vérifier le statut d'un paiement (pour polling manuel).
        Normalement, vous recevez le webhook automatiquement.
        ...
        try:
            ...
            if r.status_code == 200:
                return r.json().get('status')  # SUCCESSFUL, FAILED, PENDING
            return None
...
class OrangeMoneyClient:
    """
    Client pour Orange Money WebPay Cameroun.

    DOCUMENTATION :
      https://developer.orange.com/apis/om-webpay-cm/getting-started

    DIFFÉRENCE AVEC MTN :
      Orange redirige l'utilisateur vers une page web
      (pas de USSD push). Plus adapté au web.
    """
    ...
    def get_access_token(self):
        """Token OAuth2 pour Orange Money."""
        if not self.client_id or not self.client_secret:
            logger.error("Orange Money : CLIENT_ID ou CLIENT_SECRET manquant")
            return None
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.error(f"Orange token erreur {r.status_code}: {r.text[:200]}")
                return None

        except requests.RequestException as e:
            logger.error(f"Orange token exception: {e}")
            return None
    ...
    def initiate_payment(self, montant, telephone, reference_interne):
        ...
        RETOUR :
          {
            'succes'     : True,
            'payment_url': 'https://...' ← URL de paiement Orange
            'message'    : '...'
          }

        Le frontend doit rediriger l'utilisateur vers payment_url.
        Orange gère l'interface de paiement.
        Après paiement, Orange appelle votre webhook.
        """
        token = self.get_access_token()
        if not token:
            return {
                'succes' : False,
                'message': 'Impossible d\'obtenir le token Orange. Vérifiez ORANGE_CLIENT_ID et ORANGE_CLIENT_SECRET.'
            }

        order_id = str(reference_interne).replace('-', '')[:20]  # Max 20 chars
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.info(f"Orange Money initié: order_id={order_id}")
                return {
                    'succes'     : True,
                    'payment_url': payment_url,
                    'notif_token': data.get('notif_token', ''),
                    'message'    : f"Redirection vers Orange Money",
                }
            else:
                logger.error(f"Orange initiate erreur {r.status_code}: {r.text[:200]}")
                return {'succes': False, 'message': f"Erreur Orange ({r.status_code})."}

        except requests.Timeout:
            return {'succes': False, 'message': 'Orange Money : timeout.'}
        except requests.RequestException as e:
            return {'succes': False, 'message': f'Erreur réseau Orange: {str(e)}'}
